...
function qinpelAddAction(element, action) {
    qinpelLog("ADDED");
    element.onkeyup = keyCaller;
    element.ondblclick = clickCaller;
    element.ontouchstart = touchCaller;

    function keyCaller(event) {
        if (event.keyCode === 13) {
            qinpelLog("KEY");
            qinpelLog(action);
            action(event);
    ...
    function clickCaller(event) {
        qinpelLog("CLICK");
        action(event);
        return qinpelStopEvent(event);
    }

    function touchCaller(event) {
        // TODO - Must check if had two touch in a short period of time.
        action(event);
        return qinpelStopEvent(event);
    }
}

function qinpelAddClick(element, action) {
    element.onclick = clickCaller;
    element.ontouchstart = clickCaller;
...
function qinpelUtils() {

    function getAxiosErrorMessage(error) {
        var message = "Problem: " + error;
    ...
    return {
        addAction: qinpelAddAction,
        addClick: qinpelAddClick,
